...
struct server_response {
    ...
    void send(server_task_result & result, int outb_id) {
        ...
        printf("sending result for task id = %d\n", result.id);
...
struct server_context {
    ...
    std::vector<server_task> create_tasks_inference(json data, server_task_inf_type inf_type, int shm_id = -1) {
...
void handle_completions_generic(server_task_inf_type inf_type, json & data, httplib::Response & res, int shm_id = -1) {
    if (ctx_server.params.embedding) {
        res_error(res, format_error_response("This server does not support completions. Start it without `--embeddings`", ERROR_TYPE_NOT_SUPPORTED));
        return;
    }

    std::vector<server_task> tasks = ctx_server.create_tasks_inference(data, inf_type,shm_id);
    ...
    if (!stream) {
        ...
        const auto chunked_content_provider = [task_ids](size_t, httplib::DataSink & sink) {
        ...
        auto on_complete = [task_ids] (bool) {
            ctx_server.queue_results.remove_waiting_task_ids(task_ids);
        };
...
*/
   ...
   /* svr->set_error_handler([&res_error](const httplib::Request &, httplib::Response & res) {
    ...
    const auto handle_slots_save = [&params](const httplib::Request & req, httplib::Response & res, int id_slot) {
    ...
    const auto handle_slots_restore = [&params](const httplib::Request & req, httplib::Response & res, int id_slot) {
        json request_data = json::parse(req.body);
        std::string filename = request_data.at("filename");
        if (!fs_validate_filename(filename)) {
            res_error(res, format_error_response("Invalid filename", ERROR_TYPE_INVALID_REQUEST));
    ...
    const auto handle_props_change = [](const httplib::Request & req, httplib::Response & res) {
    ...
    const auto handle_chat_completions = [&params, verbose](const httplib::Request & req, httplib::Response & res) {
        if (ctx_server.params.embedding) {
            res_error(res, format_error_response("This server does not support completions. Start it without `--embeddings`", ERROR_TYPE_NOT_SUPPORTED));
            return;
        }
        ...
        if (!stream) {
            ...
            const auto chunked_content_provider = [task_ids, completion_id](size_t, httplib::DataSink & sink) {
                ctx_server.receive_cmpl_results_stream(task_ids, [&](const server_task_result & result) -> bool {
    ...
    const auto handle_models = [&params](const httplib::Request &, httplib::Response & res) {
        json models = {
            {"object", "list"},
            {"data", {
                {
...
*/
    ...
    if (!ctx_server.load_model(params)) {
        //clean_up();
        clean_shm((SharedMemory*)comm.requestQueue);
        //t.join();
